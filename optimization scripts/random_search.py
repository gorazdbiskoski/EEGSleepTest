...
ax_conv.set_title('Random Search – Convergence')
...
def _update_convergence(trial, best_loss):
    ...
    fig_conv.savefig(os.path.join(VIZ_DIR, 'random_search_convergence.png'), dpi=100)
# ──────────────────────────────────────────────────────────────────────────────
...
def run_random_search(num_trials=20):
    ...
    for i in range(num_trials):
        ...
        results_log.append({
            ...
            "val_loss": loss, "method": "Random Search"
        })
...
def append_best_to_summary(best_params, best_loss, elapsed):
    ...
    row = pd.DataFrame([{
        "filters": f, "kernel_size": k, "lstm_units": u,
        "dropout": d, "learning_rate": lr, "batch_size": b,
        "val_loss": best_loss, "method": "Random Search",
